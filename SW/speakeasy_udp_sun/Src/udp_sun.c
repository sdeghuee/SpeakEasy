...
//   struct udp_pcb *upcb_rx;
   ...
   if (upcb)
   {
     ...
     /* Using IP_ADDR_ANY allow the upcb to be used by any local interface */
      err = udp_bind(upcb, IP_ADDR_ANY, UDP_SECOND_PORT);
...
void udp_receive_callback(void *arg, struct udp_pcb *upcb, struct pbuf *p, const ip_addr_t *addr, uint16_t port) {
    ...
    uint16_t playbackControl = ((uint16_t *) p->payload)[1];
    HAL_GPIO_WritePin(ADC_Switch_GPIO_Port, ADC_Switch_Pin, playbackControl);
    bufferReceived[bufferCheck] = 1;
    if (bufferCheck < 1) {
    	bufferCheck += 10;
    }
    if (!bufferReceived[bufferCheck - 1]) {
		udp_scratch_send(&txBuffer[(bufferCheck - 1) * BUFF_SIZE], BUFF_SIZE, bufferCheck - 1);
    }
	pbuf_free(p);
}
